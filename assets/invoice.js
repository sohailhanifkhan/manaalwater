...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function buildInvoiceMessage(order, documentId) {
    ...
    const itemsText =
      items.length
        ...
        : `${order.qty || 1} × ${order.productName || 'Item'}`;
    ...
    if (
      ...
      lines.push(
        `*Subtotal:* Rs. ${order.subtotalAmount.toLocaleString('en-PK')}`
      );
    ...
    if (
      ...
      lines.push(
        `*Discount:* - Rs. ${order.discountAmount.toLocaleString('en-PK')}`
      );
    ...
    if (
      ...
      lines.push(
        `*Delivery Fee:* Rs. ${order.deliveryFee.toLocaleString('en-PK')}`
      );
