...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Hero video — doesn't autoplay, so its original audio can play normally.
  // Browsers only allow audible playback after a real click, which is
  ...
  if (heroVideo && heroPlayToggle) {
    ...
    heroVideo.addEventListener('play', () => { heroPlayToggle.classList.add('is-playing'); });
    heroVideo.addEventListener('pause', () => { heroPlayToggle.classList.remove('is-playing'); });
    heroVideo.addEventListener('ended', () => { heroPlayToggle.classList.remove('is-playing'); });
  }
  ...
  // Auto-fill delivery details for logged-in customers (order page only)
  // Wrapped defensively — if Firebase fails to load for any reason (slow
  // connection, blocked script, etc.), the rest of this page must keep working.
  try {
    const nameField = document.getElementById('name');
    if (nameField && typeof auth !== 'undefined') {
      auth.onAuthStateChanged(async (user) => {
        ...
        try {
          ...
          if (doc.exists) {
            ...
            if (nameField && !nameField.value) nameField.value = d.name || '';
            if (phoneField && !phoneField.value) phoneField.value = d.phone || '';
            if (addressField && !addressField.value) addressField.value = d.address || '';
          }
        } catch (e) {
          console.warn('Could not auto-fill from saved profile', e);
        }
      });
    }
  } catch (e) {
    console.warn('Account auto-fill unavailable (Firebase did not load):', e);
  }

  // Order form — multi-item cart + payment method live summary
  ...
  let liveProducts = [];
  let cartQty = []; // parallel array to liveProducts — quantity of each, 0 = not in cart

  // Promo codes — simple client-side list. FRESH10 = 10% off, matches the
  // "10% off your first order" banner shown across the site.
  const PROMO_CODES = { 'FRESH10': 10 };
  let appliedPromo = null; // { code, percent } or null

  function cartLines() {
    return liveProducts
      .map((p, i) => ({ product: p, qty: cartQty[i] || 0 }))
  ...
  function updateSummary() {
    ...
    let subtotal = 0;
    summaryItemsEl.innerHTML = lines.map(({ product, qty }) => {
      const lineTotal = product.price * qty;
      subtotal += lineTotal;
      return `<div class="summary-row"><span>${qty} × ${product.name} (${product.size})</span><span>Rs. ${lineTotal.toLocaleString()}</span></div>`;
    }).join('');

    let discountValue = 0;
    if (appliedPromo) {
      discountValue = Math.round(subtotal * (appliedPromo.percent / 100));
      if (discountRow && discountAmountEl && discountLabelEl) {
        discountLabelEl.textContent = appliedPromo.code;
        discountAmountEl.textContent = '− Rs. ' + discountValue.toLocaleString();
    ...
    const grandTotal = subtotal + deliveryFee - discountValue;
  ...
  if (applyPromoBtn) {
    applyPromoBtn.addEventListener('click', () => {
      ...
      if (PROMO_CODES[code]) {
        ...
        promoMessage.textContent = `✓ ${PROMO_CODES[code]}% discount applied!`;
        promoMessage.style.color = '#0a8a76';
      } else {
        appliedPromo = null;
        promoMessage.textContent = 'That code isn\'t valid.';
        promoMessage.style.color = '#a33';
      }
      updateSummary();
    });
  }
  ...
  function renderProductGrid() {
    ...
    productGrid.innerHTML = liveProducts.map((p, i) => {
      ...
      return `
        <div class="product-option${qty > 0 ? ' has-qty' : ''}" data-index="${i}">
          ...
          <div class="p-right">
            ...
            <div class="cart-stepper">
              <button type="button" class="cart-step" data-index="${i}" data-step="-1" aria-label="Remove one">−</button>
              <span class="cart-qty" data-index="${i}">${qty}</span>
              <button type="button" class="cart-step" data-index="${i}" data-step="1" aria-label="Add one">+</button>
            </div>
          </div>
        </div>`;
    }).join('');

    productGrid.querySelectorAll('.cart-step').forEach(btn => {
      btn.addEventListener('click', () => {
        const i = parseInt(btn.dataset.index, 10);
        const step = parseInt(btn.dataset.step, 10);
        cartQty[i] = Math.max(0, (cartQty[i] || 0) + step);
        renderProductGrid();
        updateSummary();
      });
    });
  ...
  function buildOrderMessage(mapsLink) {
    ...
    const itemsText = lines.length
      ? lines.map(({ product, qty }) => `  • ${qty} × ${product.name} (${product.size}) — Rs. ${(product.price * qty).toLocaleString()}`).join('\n')
      : '  • (no items selected)';
    ...
    msg += `*Items:*\n${itemsText}\n`;
    if (appliedPromo) {
      msg += `*Promo Code:* ${appliedPromo.code} (${appliedPromo.percent}% off)\n`;
    }
    msg += `*Total:* ${total}\n`;
  ...
  function saveOrderToFirestore(mapsLink) {
    ...
    try {
      ...
      }).catch(e => console.warn('Order save to Firestore failed (WhatsApp order still sent):', e));
    } catch (e) {
      console.warn('Order save to Firestore failed (WhatsApp order still sent):', e);
    }
  }
  ...
  function sendToWhatsApp(mapsLink) {
    ...
    // empty in some installed-app/standalone contexts. Direct navigation
    // always carries the message through correctly.
  ...
  if (orderForm) {
    orderForm.addEventListener('submit', (e) => {
      ...
      if (cartLines().length === 0) {
        alert('Please add at least one item using the + button before placing your order.');
        return;
      }
      if (placeOrderBtn) {
