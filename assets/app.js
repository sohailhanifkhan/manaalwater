...
// ------------------------------------------------------------
// Manaal Water — Google Sheets Sales Register Sync
// Firestore remains the primary order database.
// This sends a copy of successfully saved orders to Google Sheets.
// ------------------------------------------------------------
...
async function syncOrderToSalesSheet(
  ...
  try {
    ...
    // text/plain avoids a browser CORS preflight.
    // no-cors + keepalive allows the request to continue
    // even when checkout immediately opens WhatsApp.
    await fetch(
    ...
    // Spreadsheet sync must never stop a customer order.
    console.warn(
...
msg +=
  `*Delivery Instructions:* ${deliveryInstructions}\n`;
    ...
    async function saveOrderToFirestore(
      ...
      // Firestore remains the primary order record.
      // Google Sheets is synchronized only after Firestore succeeds.
      ...
      try {
        ...
        // Generate the Firestore ID before writing.
        // The same ID becomes the spreadsheet's unique key.
        const docRef =
          db.collection(
            'orders'
          ).doc();
        ...
        console.warn(
          'Order save/sync failed (WhatsApp order can still continue):',
          e
        );
